<div class="modal fade" id="editarSupervisorModal" tabindex="-1" aria-labelledby="editarSupervisorModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editarSupervisorModalLabel">Editar Supervisor</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="editarSupervisorForm" action="procesarSupervisor.php" method="post">
                    <input type="hidden" name="action" value="editar">
                    <input type="hidden" id="id_supervisor_editar" name="id_supervisor">
                    <div class="mb-3">
                        <label for="nombre_editar" class="form-label">Nombre</label>
                        <input type="text" class="form-control" id="nombre_editar" name="nombre" required>
                    </div>
                    <div class="mb-3">
                        <label for="dni_editar" class="form-label">DNI</label>
                        <input type="text" class="form-control" id="dni_editar" name="dni" required>
                    </div>
                    <div class="mb-3">
                        <label for="mail_editar" class="form-label">Email</label>
                        <input type="email" class="form-control" id="mail_editar" name="mail" required>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cerrar</button>
                        <button type="submit" class="btn btn-primary">Guardar Cambios</button>
                    </div>
                </form>
            </div>
        </div>
    </div>
</div>
