...
<div class="main p-3 ms-5">
    ...
    echo "<div class='mb-3'>
            <a href='?mes=$mes_anterior&año=$año_anterior' class='btn btn-primary'>&laquo; Mes Anterior</a>
            <span class='mx-3'>Mes: " . date('F', mktime(0, 0, 0, $mes_actual, 10)) . "</span>
            <a href='?mes=$mes_siguiente&año=$año_siguiente' class='btn btn-primary'>Mes Siguiente &raquo;</a>
          </div>";
    ...
    $consulta_vendedores = "SELECT e.id, e.nombre AS vendedor, z.id AS zona_id, z.nombre AS zona, SUM(v.monto) AS total_vendido, 
                            CASE 
                                WHEN SUM(v.monto) > 100 THEN SUM(v.monto) * 0.10
                                ELSE 0
                            END AS premio
                            FROM ventas v
                            JOIN empleados e ON v.vendedor_id = e.id
                            JOIN zonas z ON e.zona_id = z.id
                            WHERE MONTH(v.fecha) = $mes_actual AND YEAR(v.fecha) = $año_actual
                            GROUP BY e.id, z.id";
